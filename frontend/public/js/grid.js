...
export async function initGrid(container) {
    container.innerHTML = `
...
function drawGrid(canvas) {
    const c = canvas ?? document.getElementById('grid-canvas');
    ...
    for (let row = 0; row < layout.rows; row++) {
        for (let col = 0; col < layout.cols; col++) {
            ...
            const color = cell ? getCellColor(cell.type) : getCellColor('vide');
            ctx.fillStyle = color;
...
function bindCanvasEvents(canvas, cellSize) {
